...
function Admin() {
  ...
  const [newsForm, setNewsForm] = useState({
    title: '', category: 'Campus', summary: '', content: '', author: 'SUZUMIYA',
  ...
  const fetchBroadcast = async () => {
    const { data } = await supabase.from('broadcast').select('*').eq('id', 1).single();
    if (data) {
  ...
  const handleBroadcast = async (status) => {
    ...
    try {
      const { error } = await supabase
        .from('broadcast')
        .update({ message: broadcastMsg, is_active: status })
        .eq('id', 1);
      if (error) throw error;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-20 px-6 transition-colors duration-500">
      <div className="max-w-2xl mx-auto">
        <header className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1 uppercase tracking-widest font-bold font-mono">NERV ACCESS GRANTED</p>
          </div>
          ...
          {['news', 'alumni', 'broadcast'].map(tab => (
            <button
              ...
              className={`flex-1 py-3 rounded-xl text-[10px] md:text-sm font-black transition-all uppercase ${
                activeTab === tab ? 'bg-white dark:bg-slate-800 dark:text-white shadow-sm' : 'text-slate-500'
              }`}
            >
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-xl border border-slate-100 dark:border-slate-800">
          {activeTab === 'news' && (
            <div className="space-y-4">
              ...
              <textarea placeholder="摘要" className="admin-input h-24" value={newsForm.summary} onChange={e => setNewsForm({...newsForm, summary: e.target.value})} />
              ...
              <div className="upload-box"><input type="file" onChange={e => setNewsFile(e.target.files[0])} className="text-xs text-slate-500" /></div>
              <button onClick={handleNewsPublish} disabled={loading} className="publish-btn">{loading ? '📡 同步轨道...' : '🚀 发布新闻'}</button>
            </div>
          )}
          ...
          {activeTab === 'alumni' && (
            <div className="space-y-4">
              ...
              <textarea placeholder="校友名言" className="admin-input h-32" value={alumniForm.quote} onChange={e => setAlumniForm({...alumniForm, quote: e.target.value})} />
              <div className="upload-box"><input type="file" onChange={e => setAlumniFile(e.target.files[0])} className="text-xs text-slate-500" /></div>
          ...
          {activeTab === 'broadcast' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                <h3 className="font-black text-slate-900 dark:text-white text-sm uppercase italic tracking-widest text-red-600">MAGI System Status</h3>
                <span className={`text-[10px] font-bold px-3 py-1 rounded-full ${broadcastActive ? 'bg-red-600 text-white animate-pulse' : 'bg-slate-200 dark:bg-slate-800 text-slate-500'}`}>
              ...
              />
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => handleBroadcast(true)} disabled={loading} className="publish-btn bg-red-600 hover:bg-red-500 shadow-red-600/20">开启警报</button>
                <button onClick={() => handleBroadcast(false)} disabled={loading} className="publish-btn bg-slate-800 hover:bg-slate-700 shadow-none">解除指令</button>
              </div>
            </div>
          )}
        </div>
        ...
        {activeTab !== 'broadcast' && (
          <div className="mt-12 space-y-3">
            <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Manage Records</h2>
            {(activeTab === 'news' ? newsList : alumniList).map(item => (
              <div key={item.id} className="flex justify-between items-center p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 transition-all hover:border-orange-200">
                <div>
                  <h3 className="font-bold dark:text-white text-sm">{item.title || item.name}</h3>
                  <p className="text-[9px] opacity-40 dark:text-white uppercase mt-1">{(item.category || item.role) || '-'} • {item.created_at ? new Date(item.created_at).toLocaleDateString() : '-'}</p>
                </div>
                <button onClick={() => handleDelete(activeTab === 'news' ? 'news' : 'alumni', item.id)} className="text-red-600 font-black text-[10px] uppercase hover:bg-red-600 hover:text-white px-4 py-2 rounded-xl transition-all">Delete</button>
              </div>
            ))}
          </div>
        )}
      ...
      <style jsx>{`
        .admin-input { width: 100%; padding: 1rem; border-radius: 1rem; border: 1px solid #e2e8f0; background: transparent; outline: none; transition: all 0.3s; font-size: 0.875rem; }
        .dark .admin-input { border-color: #1e293b; color: white; background: #0f172a; }
        .admin-input:focus { border-color: #ea580c; box-shadow: 0 0 0 2px rgba(234, 88, 12, 0.1); }
        .upload-box { padding: 1rem; border: 2px dashed #e2e8f0; border-radius: 1rem; }
        .dark .upload-box { border-color: #1e293b; }
        .publish-btn { width: 100%; padding: 1.25rem; background: #ea580c; color: white; border-radius: 1.25rem; font-weight: 900; text-transform: uppercase; letter-spacing: 0.1em; transition: all 0.3s; box-shadow: 0 10px 20px -5px rgba(234, 88, 12, 0.3); }
